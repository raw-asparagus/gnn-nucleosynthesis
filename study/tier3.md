...
- Measured project numbers quoted here originate in `RESULTS.md` with script +
  ...
  wins.
- **Bibliographic details are from memory** and tagged **[sourced — verify]**.
  The physics content is what matters for study; volume and page numbers must be
  checked against the actual papers before any of it reaches `docs/` or a paper
  draft. Part 0 leans on the CCSN and nucleosynthesis literature more heavily
  than any earlier tier, so this caveat carries more weight here than anywhere
  else in the study notes.
...
> ~10⁴ km sphere sets a single scalar — Yₑ — that propagates through a collapse,
...
A spherically symmetric star in mechanical balance satisfies
...
2. **The star must supply that pressure from *something*.** Which something —
   ...
   evolution, and in silicon burning it is degenerate electrons at ~93% of the
   total (Tier 0 §0.1.2), which is exactly why Yₑ matters.

### 0.1.2 The virial theorem, and the negative heat capacity — derived

Take the hydrostatic equation, multiply by 4πr³, and integrate over the star:
...
negative heat capacity vanishes. **That is not a curiosity — it is the collapse
condition**, and §0.5.1 is the same statement with the corrections restored.
...
~8 M⊙ reach silicon burning at all — below that, electron degeneracy (§0.1.4)
halts contraction before the core is hot enough.
...
Numerically, for a 1.4 M⊙ core with μ̄ ≈ 1.4 (silicon-ash composition, mildly
ionised bookkeeping): [derived here]
...
~10⁸ g cm⁻³ is at silicon-burning temperature.** That is the regime box's
centre, and it was not chosen — it is where the star puts itself. The box
(T₉ 1.6–7.9, ρ 10⁷–10⁹, Tier 0 §III.2) is a one-decade window around this line
in each variable.

The estimate is crude by a factor of ~2 in T (the ideal-gas assumption is already
wrong at 10⁸ g cm⁻³, where electrons are degenerate — §0.1.4), and it should not
be quoted as a prediction. It is here to show that the regime box is *derivable*
from M, G and k, not sampled from a paper.

### 0.1.4 Degeneracy: the wall the star runs into

Electron degeneracy pressure is independent of temperature, so a degenerate core
does **not** heat when it contracts — the virial argument above assumed an ideal
gas and fails. Whether a core is degenerate is decided by comparing the electron
Fermi energy with kT:
...
At T₉ = 4 (kT = 0.345 MeV): [derived here]
...
non-degenerate and its high-density edge is not. That single fact is why the box
...
a white dwarf. That is the fate of everything below ~8 M⊙. Above that mass the
...
Tier 1 §1.2 derives the Gamow peak: for a non-resonant charged-particle reaction
between nuclei of charges Z₁, Z₂ and reduced mass μ, the effective burning energy
and the rate's temperature dependence are governed by
...
\;=\; 0.9796\,\left(Z_1Z_2\right)^{2}A_{\mathrm{red}}\ \mathrm{MeV},
\qquad
\tau \;\equiv\; \frac{3E_0}{kT} \;=\; 3\left(\frac{E_G}{4kT}\right)^{1/3},
$$

so that ⟨σv⟩ ∝ e^{−τ}. Ignition happens when the rate becomes fast enough to
...
stage's actual ignition temperature: [derived here]

| fuel | dominant channel | Z₁Z₂ | A_red | E_G [MeV] | T_ign (actual) | τ_ign |
|---|---|---|---|---|---|---|
| H (pp) | p + p | 1 | 0.50 | 0.49 | 0.004 GK | 21.3 |
| H (CNO) | ¹⁴N(p,γ)¹⁵O | 7 | 0.93 | 44.8 | 0.03 GK | 48.9 |
| He | α + α (→ ¹²C) | 4 | 2.0 | 31.3 | 0.2 GK | *resonant* |
| C | ¹²C + ¹²C | 36 | 6.0 | 7 617 | 0.8 GK | 90.7 |
| Ne | ²⁰Ne(γ,α) | — | — | — (**photo**) | 1.5 GK | — |
| O | ¹⁶O + ¹⁶O | 64 | 8.0 | 32 100 | 2.0 GK | 107.9 |
| Si | ²⁸Si(γ,α) | — | — | — (**photo**) | 3–4 GK | — |

The non-resonant charged-particle rows reproduce their ignition temperatures to
better than 1% as *ratios* of E_G/τ³ — pp → CNO predicts 7.50 against an actual
7.5, pp → C predicts 200 against 200, pp → O predicts 500 against 500. And the
mechanism is now visible: **E_G spans a factor of 6.6 × 10⁴ across those four
rows, τ³ spans 130, and the quotient is the observed 500.** The Coulomb barrier
really is enormous; the reason the staircase is compressed into two decades of
temperature rather than five is that the star simply burns deeper into the
exponential tail for each successive fuel.
...
Hoyle resonance in ¹²C, so its rate is set by a narrow-resonance formula, not by
the non-resonant Gamow integral; its τ is not the controlling quantity and the
...
acquires enough γ-rays above the (γ,α) threshold. Take the same estimate
seriously for ²⁸Si + ²⁸Si: Z₁Z₂ = 196 and A_red = 14 give E_G = 5.3 × 10⁵ MeV,
16.4× oxygen's, and extrapolating the slowly-rising τ_ign ≈ 115–130 puts
fusion ignition at **≈ 20 GK** (19–27 GK across that τ range) — far above where
the star ever gets in hydrostatic burning, and above the temperature at which the
Fe peak itself dismantles.
...
the mass excesses, Tier 0 §0.3.1] Between those two temperatures the Boltzmann
suppression changes by a factor e^{9.7} ≈ 1.6 × 10⁴. The transition from "no
...
N_A × (ΔB/A) [MeV] × 1.602 × 10⁻⁶ erg/MeV. [derived here]

| stage | representative reaction | ΔB [MeV] | per nucleon [MeV] | per gram [erg/g] |
|---|---|---|---|---|
| H → He | 4p → ⁴He + 2e⁺ + 2ν | 26.7 | 6.7 | 6.5 × 10¹⁸ |
...
the curve: ⁵⁶Fe 8.790, ⁶²Ni 8.795, ⁵⁶Ni 8.643 MeV/nucleon. [derived here, from
the same mass table] ⁶²Ni is the maximum, not ⁵⁶Fe — the standard shorthand is
false, and Tier 2 §0.2.3 explains why ⁵⁶Fe nonetheless wins in NSE at
...
emission from the plasma, not photon radiation from the surface.** The contrast
is in *how the energy gets out*, not in how long a fuel lasts. Photons are
trapped: the interior is optically thick, so energy random-walks outward on a
thermal (Kelvin–Helmholtz) timescale of ~10⁴ years for a massive star, is
...
and mean free paths vastly exceeding the stellar radius, so they leave **at the
speed of light, directly from the point of production** (Tier 0 §0.4.2), and the
...
hydrogen because there is a great deal of hydrogen and §0.2.2's release per gram
is large, not because of any transport time. What the transport argument
...
e⁺e⁻ → νν̄. Its emissivity requires a thermal positron population, which requires
kT comparable to m_e c² = 0.511 MeV, i.e. T₉ ≳ 1. Once that threshold is crossed
the positron number density rises as roughly exp(−2m_ec²/kT) times phase-space
factors, and the emissivity per gram rises extremely steeply — approximately as
T⁹ in the relevant band (Tier 0 §I.1) [sourced — verify: Itoh et al. 1996, ApJS
102, 411, is the standard fitting reference].
...
The runaway is not in the burning; it is in the **cooling**. Each stage must
generate energy at the rate the neutrinos remove it, and that rate rises by nine
powers of T while the available fuel energy per gram (§0.2.2) is roughly
constant. So the duration of stage n scales as

$$
\tau_n \;\sim\; \frac{E_{\mathrm{fuel}}}{\varepsilon_\nu(T_n)} \;\propto\; T_n^{-9}
$$

to leading order. Check it: from oxygen burning (T₉ ≈ 2, months) to silicon
burning (T₉ ≈ 3.5), the temperature ratio is 1.75 and 1.75⁹ = 154. Months / 154
is of order a day. **The one-day silicon-burning timescale is a T⁹ scaling law
applied to the oxygen-burning duration.** [derived here — order-of-magnitude
only; the actual duration depends on core mass and shell structure and ranges
over roughly hours to a few days across progenitors [typical]]
...
To ground the above: at silicon-burning conditions, the star's core is a
~10⁴ km sphere of ~1.4 M⊙ at ρ ~ 10⁸ g cm⁻³ and T ~ 3.5 GK, converting silicon
to iron-peak nuclei over about a day, radiating almost all of that energy as
neutrinos that leave instantly, contracting steadily because the losses exceed
the generation, and building an iron core that grows toward a mass limit it
cannot exceed.

Meanwhile, in a MESA calculation of the same object, the nuclear network is being
called once per zone per timestep — 10³–10⁴ zones, and timesteps so short at the
end that the last day of the star's life can consume a large fraction of the
total run time of the whole 10⁷-year calculation. §0.8 quantifies that.
...
**Silicon shell burning is convective.** The energy generation rate is a steep
function of temperature, so the burning region is strongly superadiabatic and
convection sets in immediately. In a 1D stellar-evolution code this is modelled
by mixing-length theory (MLT): convection becomes a diffusive process with
diffusivity D ≈ ⅓ v_conv ℓ, and the composition equation acquires a transport
term:
...
Those two terms are **operator-split** from each other, exactly as the network is
split from the hydrodynamics (§0.8.1). A real MESA timestep burns each zone at
fixed (T, ρ), then mixes the resulting compositions across the convective region,
then solves the structure. The emulator replaces the first operator only.
...
composition. Da ≫ 1 means burning wins and gradients survive; Da ≪ 1 means mixing
wins and the region is homogeneous in that quantity. Read the numbers off Tier 0
...
**And the modelling caveat that tempers all of it.** MLT is known to describe
late-stage convection poorly. Three-dimensional hydrodynamic simulations of
oxygen and silicon shell burning show turbulent entrainment at convective
boundaries, large-scale bulk motions at Mach numbers approaching 0.1, and — in
some models — **shell mergers**, where the oxygen and neon shells convectively
merge and the composition changes qualitatively [sourced — verify: the standard
3D shell-burning results, e.g. Meakin & Arnett; Müller and collaborators]. Shell
mergers change the pre-supernova structure, the compactness and the yields, and
1D codes reproduce them only through calibrated prescriptions.

So the honest hierarchy of modelling error in a pre-supernova model is
**3D convection ≫ network size ≳ rate uncertainties ≫ emulator target** — §0.6.6
assembles it properly, because it determines what the project can claim.
...
Start from a zone of ²⁸Si and ³²S (the ash of oxygen burning) at T₉ ≈ 3.5,
ρ ≈ 10⁸.
...
Nuclei with the *lowest* particle-separation energies are attacked first: the
(γ,α), (γ,p) and (γ,n) channels on whichever species in the mixture happens to be
least bound against that channel. For a silicon-group mixture, that is not
²⁸Si — ²⁸Si is comparatively tightly bound — but the odd-A and less-bound
neighbours.
...
achieved by disassembling some fraction of the silicon completely and reassembling
it. No ²⁸Si nucleus ever touches another ²⁸Si nucleus.
...
sector, which is a small fraction of the columns. That asymmetry is the project's
central bet.
...
connected to each other only through the slow bridges. This is Bodansky, Clayton
& Fowler's 1968 result [sourced — verify: ApJS 16, 299], and it is the reason the
composition during silicon burning is neither a free ODE trajectory (too many
degrees of freedom) nor a state function (NSE — too few).
...
[derived here, from binding energies] — so ⁴⁴Ti photodisintegrates *more* readily
than the ⁴⁰Ca below it, and the (γ,α) reverse on the ⁴⁰Ca(α,γ)⁴⁴Ti step runs
hard. The bridge is slow in **net** terms precisely because its reverse is fast:
material pushed across is pushed straight back. A local dip in the
binding-energy-per-nucleon curve (⁴⁴Ti at 8.534 MeV/nucleon, *below* ⁴⁰Ca's
8.551) is what a bottleneck looks like from the thermodynamic side.
...
**And therefore the boundary is not sharp.** Whether ⁴⁵Sc counts as
silicon-group or iron-group is a *modelling choice about a continuum*. The repo
...
high-Yₑ bottleneck of silicon burning [sourced — verify: Grichener et al. 2025,
ApJS 279, 49, and references therein], and this project found it independently at
rank 2 of 251 inter-group carriers on relaxed high-Yₑ trajectory rows in the
3.3–5 GK window. That is a genuine external validation of the whole flux
machinery: a published physical claim, reproduced from first principles by an
independent pipeline. It is also a warning about distribution — on the *training*
...
ΣXᵢ = 1 and Σ(Zᵢ/Aᵢ)Xᵢ = Yₑ. At low enough temperature that the binding-energy
term dominates the entropy term, the winner is the nucleus that maximises binding
energy per nucleon **subject to having the right Z/A**. Since Yₑ = ⟨Z/A⟩ for the
...
| 0.482 | 26/54 = 0.4815 | **⁵⁴Fe** |
| 0.464 | 26/56 = 0.4643 | **⁵⁶Fe** (B/A = 8.790) |
| 0.452 | 28/62 = 0.4516 | ⁶²Ni region (⁵⁸Fe, at 26/58 = 0.448, sits just below) |

[derived here — the Z/A arithmetic; the NSE dominance statement is the standard
result and the repo's own Saha solver reproduces it, e.g. **[RESULTS]**
2026-07-11 quotes NSE fe56 = 0.69–0.72 at Yₑ ≈ 0.467.]

So the composition arriving at the iron peak is a **direct readout of Yₑ**, and
the ⁵⁶Ni mass — the thing that powers the supernova light curve — is set by how
much of the burnt material stayed near Yₑ = 0.5.
...
core burns for ~a day at T₉ ≈ 3–4, ρ ≈ 10⁷–10⁹, cooling by neutrinos, in
...
through the silicon and oxygen shells. Conditions: peak T₉ ≈ 4–5+, ρ ≈ 10⁷–10⁸,
and a duration set by the *hydrodynamic expansion timescale* — of order
0.1–1 s [typical]. The material is heated, burns, and then expands and cools on
that same timescale.
...
**The weak-freeze difference is the important one.** Electron-capture rates at
these conditions are of order 10⁻²–10⁰ s⁻¹ per nucleus [typical — set by the
FFN/LMP tables, Tier 1 §V]. Over 10⁵ s that changes Yₑ by a lot; over 0.1 s it
changes it by ~10⁻² at most and often much less. So **explosive burning largely
inherits the Yₑ that hydrostatic burning produced**, and the ejecta composition
...
Material shocked to **high temperature and low density** (T₉ ≳ 5,
ρ ≲ 10⁷ g cm⁻³ [typical]) reaches full NSE, which at those conditions is
dominated by free α particles and nucleons rather than by the iron peak — high
temperature favours the high-entropy, many-particle state. As the material then
expands and cools, the composition tries to follow NSE back down toward the iron
peak. Reassembling α particles into heavy nuclei requires the **triple-α
reaction** (and ⁴He(αn,γ)⁹Be), which is a three-body process and therefore has a
rate ∝ ρ². At low density that rate cannot keep up with the expansion.

The result is a **freeze-out with leftover α particles** — typically 10⁻²–10⁻¹
by mass [typical] — which then capture onto the iron-peak nuclei that did form.
Consequences:

- extra ⁴⁴Ti (an α-capture product; observable, §0.6.4);
- shifted iron-peak isotopic ratios relative to a normal freeze-out;
- a composition that is *not* NSE and never was, despite having passed through
  NSE conditions.
...
**Stellar-evolution codes stop integrating the network at high temperature.**
Above roughly 5–7 GK (the exact threshold is a code parameter [typical]) the
strong reactions are so fast that the ODE system becomes maximally stiff, the
composition is a function of state to high accuracy, and integrating it is both
expensive and pointless. The standard response is to switch to an **NSE solve or
table**: compute X(T, ρ, Yₑ) from Saha, and evolve only Yₑ through the weak
reactions evaluated on that composition.
...
| what is integrated | dY/dt = νR | dYₑ/dt = Σ_weak (…) evaluated at X_NSE |
| cost | stiff implicit solve | one Newton solve on 2 unknowns |
| stiffness | 10¹²–10¹⁵ | none |

Four consequences the project should be carrying and is not.

**(i) The emulator's competitor above the threshold is not a network — it is a
Saha solve.** And a Saha solve is *fast*: this project's own batched solver does
27 states with no fallback and agrees with pynucastro to 10⁻¹⁰ dex
**[RESULTS]** 2026-07-10. If the host code would use NSE at T₉ > 5 anyway, then
an emulator competing there is competing with something already cheap and
already exact. This is exactly Tier 2 **R-16** / this tier's **T-08**
(NSE-as-a-predictor as a baseline) arriving from the *deployment* side rather
than the evaluation side, and it is a strong independent argument for **T-03**'s
validity domain.

**(ii) The handoff is discontinuous, and the discontinuity is a physical
modelling error nobody in this project has sized.** At the switch temperature the
composition jumps from "whatever the network integrated" to "exactly NSE". Those
differ, because the network was *approaching* NSE, not at it (§0.3.5's freeze-out
is the general form of this). Codes handle it with hysteresis, blending, or
thresholds on the NSE distance. `tier1.md` §VIII.E.6 flags the handoff as
unspecified; the sharper statement is that **the transition is where the label
generator changes character**, and it sits inside the regime box.

**(iii) The pathology of §III lives on the wrong side of it.** The
Appendix-B-displaced labels are at T₉ ≳ 5 — i.e. in the band where a real stellar
code would likely have handed off to NSE and never called the network at all. So
the most damaged part of the dataset may also be the least deployment-relevant
part. That is a genuinely important observation for the benchmark-versus-physics
fork (§III.6): if the host code does not call a network there, training an
emulator to reproduce buggy labels there costs nothing scientifically and buys
benchmark comparability — which tips the fork toward **T-03**'s third option
decisively.

It also has a sharp corollary: **the shipped test trajectories run bbq's network
at temperatures where a stellar code would not.** bbq is a bare burner with no
NSE switch, so the dataset explores a regime the deployment path avoids.
...
channels of §0.7.3, evaluated on a Saha composition. An emulator for that regime
...
deposits it onto the core. The core mass is therefore a monotonically increasing
function of time, approaching a limit it cannot exceed.
...
burns, exhausts, contracts, reignites — and its conditions sweep through the
regime box rather than sitting at a point. The project's per-state,
...
that actually applies has two corrections.

The cold, fully relativistic limit:
...
Numerically: 1.456 M⊙ at Yₑ = 0.5, 1.179 at 0.45, 1.027 at 0.42, 0.713 at 0.35.
[derived here] The derivative is
...
with s_e the electron entropy per baryon in units of k [sourced — verify: this is
the standard Baron & Cooperstein/Bethe form]. At s_e ≈ 1 and Yₑ = 0.45 the
bracket is 1.5 — a 50% correction, not a small one. So the core mass at collapse
is a *two-parameter* function (Yₑ and entropy), and the entropy is set by the
burning history too.

**Correction 2 — general relativity and the surrounding pressure lower it.** The
real collapse threshold is not a clean crossing of M_ch; it is a stability
question about a core embedded in a star, with GR corrections, and it is decided
by the equation of state's adiabatic index (§0.5.1).
...
the **compactness parameter** [sourced — verify: O'Connor & Ott 2011, ApJ 730,
70]:

$$
\xi_{M} \;=\; \frac{M/M_\odot}{R(M)/1000\ \mathrm{km}}\bigg|_{t=\text{bounce}},
\qquad \text{usually } M = 2.5\,M_\odot .
$$

It measures how much mass sits inside a given radius — i.e. how steeply the
density falls outside the core. Its usefulness: it correlates with whether the
neutrino-driven mechanism succeeds (§0.5.7). High compactness → a lot of material
raining onto the shock at high rate → harder to explode → black hole. Low
compactness → the accretion rate drops sharply after the core → the shock
revives → neutron star and a supernova.

The compactness is set by the mass and entropy structure of the silicon and
oxygen shells at collapse, and it is famously **non-monotonic in progenitor
mass** — small changes in the shell-burning history produce large changes in
ξ_2.5, and hence in the predicted outcome. That sensitivity is *why* the
...
For orientation [typical, from the standard progenitor literature — verify
against a specific model set before quoting]:
...
- **Yₑ = 0.5 exactly** only for material that has never had a weak interaction
  ...
  the initial metallicity (¹⁴N → ²²Ne through He burning) introduces neutron
  excess.
- **Yₑ ≈ 0.85** in a surviving hydrogen envelope — hydrogen has Z/A = 1, so any
...
- **Yₑ ≈ 0.498–0.499** in the He-burnt but otherwise unprocessed layers (the
  ²²Ne-bearing C/O material).
- **Yₑ ≈ 0.49–0.50** in the oxygen shell.
- **Yₑ ≈ 0.44–0.49** in the silicon-burning region and the iron core, with the
  central value lowest.
- **Yₑ ≈ 0.42–0.43** at the centre at collapse onset in the more evolved models.
...
all non-monotonic in initial mass (§0.4.3). A conclusion drawn from a 20 M⊙ track
...
material entering carbon burning. So η at the start of silicon burning is not
zero and is metallicity-dependent. It also sets the line-driven mass-loss rate,
hence whether the hydrogen envelope survives.
...
and hence the compactness. It also produces the angular momentum that some
explosion mechanisms need.

**Binarity — the axis that changes the observable.** The majority of massive stars
are in binaries that interact [sourced — verify: the standard multiplicity
result, e.g. Sana et al. 2012]. Interaction strips the hydrogen envelope, which
converts a Type IIP into a Type Ib/c. And §0.6.1's Arnett-rule reading of ⁵⁶Ni
...
1. **The single-track comparison is a sample of size one.** **R-07**'s finding —
   ...
   visited point — is a statement about one star. Whether the union over a
...
The 4/3 is not arbitrary. §0.1.2 derived it: for a γ = 4/3 gas the virial theorem
...
Γ₁ → 4/3 in the ideal limit. A real core sits a little above that — partial
degeneracy, finite temperature and the residual electron mass all push Γ₁ up by
a few percent — while general relativity pushes the *threshold* up by
O(GM/Rc²) (§0.4.2's Correction 2). **Both margins are small, and the point
...
Two such processes are guaranteed to be present, and they are the two runaways.

### 0.5.2 The two runaways

**(a) Photodisintegration of the iron peak.** Above T ≈ 7–10 GK (strongly
density-dependent, and *above* the project's regime box) the thermal photon bath
dismantles iron-peak nuclei:
...
right. [derived here, from binding energies]
...
pressure goes into breaking nuclei instead, and Γ₁ drops below 4/3.
...
Yₑ drops, so M_ch ∝ Yₑ² drops, so the core that was marginally supportable no
longer is.

The two reinforce: dissociation makes free protons, which are the best EC
targets; EC lowers the pressure, accelerating contraction, raising T, accelerating
dissociation. Collapse becomes dynamical.
...
**The whole collapse takes a few hundred milliseconds.** Compare with the day-long
silicon-burning episode that set the initial conditions: the ratio is ~10⁵. This
extreme separation is why the collapse can be treated as taking the composition
...
falls fast. At some density they stop escaping, and Yₑ freezes. Deriving where
that happens is worth doing because the trapped Yₑ is the number that sets the
homologous core mass.

The relevant opacity is **coherent neutral-current scattering off nuclei**, whose
cross-section is enhanced by A² because the whole nucleus scatters in phase when
the neutrino wavelength exceeds the nuclear size:

$$
\sigma_{\mathrm{coh}} \;\approx\; \frac{\sigma_0}{4}\left(\frac{E_\nu}{m_ec^2}\right)^{2} \frac{A^{2}}{6},
\qquad \sigma_0 = 1.76\times10^{-44}\ \mathrm{cm}^2 .
$$

[sourced — verify: the standard weak-interaction reference cross-section and the
coherent A²/6 form; e.g. Bethe 1990, Rev. Mod. Phys. 62, 801.] For A = 56 and
E_ν = 20 MeV this gives σ ≈ 3.5 × 10⁻³⁹ cm². The mean free path is
λ = 1/(n_A σ) with n_A = ρN_A/A, and the neutrinos random-walk out of a core of
radius R ≈ 100 km in a diffusion time t_diff ≈ 3R²/(λc). Trapping happens when
t_diff exceeds the dynamical time t_ff:

| ρ [g cm⁻³] | λ [cm] | t_diff [s] | t_ff [s] | trapped? |
|---|---|---|---|---|
| 10¹¹ | 2.6 × 10⁵ | 0.038 | 0.0066 | yes (marginally) |
| 10¹² | 2.6 × 10⁴ | 0.38 | 0.0021 | firmly |

[derived here — an order-of-magnitude calculation with fixed A = 56, E_ν = 20 MeV
and R = 100 km; the real trapping density in the literature is a few × 10¹¹ g
cm⁻³ [sourced — verify], which this reproduces.] **The A = 56 choice biases the
answer in a known direction**: nuclei in the collapsing core are heavier
(A ~ 80–100), and since σ ∝ A² while n_A ∝ ρ/A, the mean free path goes as
1/(ρA). Using the real A shortens λ and traps *earlier*, so the densities in the
table are upper bounds.

**Consequences.**

1. **Yₑ freezes at trapping.** Its value there — around 0.35 [typical] — is the
   *lepton fraction* Y_L that the inner core carries through the rest of the
   collapse (the trapped neutrinos contribute their own pressure, and the
   relevant conserved quantity becomes Y_L = Yₑ + Y_ν).
2. **The homologous core mass is set by that trapped value**, through the same
   M ∝ Y_L² relation (§0.5.5).
...
self-similarly, maintaining its shape. Outside a sonic point the infall is
supersonic and cannot communicate; that outer material simply rains down.
...
which at Y_L ≈ 0.35 gives ≈ 0.7 M⊙ [derived here + typical]. Collapse continues
until the central density reaches nuclear saturation, ρ ≈ 2.7 × 10¹⁴ g cm⁻³, at
which point the nuclear force provides sudden, enormous stiffness: Γ₁ jumps well
above 4/3.

The inner core overshoots, halts, and **bounces**. The bounce launches a shock at
the *edge of the homologous core* — i.e. at a mass coordinate of ≈ 0.7 M⊙, inside
material that is still falling in at ~0.1c.
...
is set by Yₑ at collapse onset, which is set by silicon burning. Every link is a
squared or steeper dependence.
...
the star apart) does not work, and the arithmetic showing why is short and worth
doing.

The shock must traverse the outer iron core — perhaps 0.5 M⊙ of iron-peak
material [typical] — and it **dissociates every nucleus it passes through**,
because the post-shock temperature is far above the dissociation threshold. The
cost, from §0.5.2, is 8.79 MeV per nucleon:
...
[derived here] Compare with the energy available to the shock — of order
10⁵¹ erg [typical] — and with the observed explosion energy of a typical
core-collapse supernova, ~10⁵¹ erg. **The dissociation cost alone exceeds the
shock's entire energy budget by roughly an order of magnitude.** On top of that,
the dissociated free nucleons are excellent electron-capture targets, so the
shocked material rapidly deleptonises and emits a burst of neutrinos that carries
away more energy still (the "neutronisation burst", ~10⁵¹ erg in a few
milliseconds).

The prompt shock **stalls**, typically within tens of milliseconds, at a radius
of ~100–200 km, and becomes a standing accretion shock: material continues to
fall through it and pile onto the proto-neutron star.
...
i.e. ~96 MeV per baryon, or ~10% of the rest mass. [derived here] **The explosion
needs 1% of that.** The entire core-collapse supernova problem is the question of
how to transfer one percent of the available energy from the neutron star to the
overlying material.

### 0.5.7 The delayed neutrino-driven mechanism

The standard answer [sourced — verify: Bethe & Wilson 1985, ApJ 295, 14]:
the proto-neutron star radiates its binding energy as neutrinos of all flavours
over ~10 s. A small fraction of those neutrinos — of order 10⁻² — is **reabsorbed**
in the semi-transparent region behind the stalled shock (the "gain region"),
through
...
explosion succeeds. Modern simulations show the process is aided substantially by
multi-dimensional hydrodynamic instabilities (convection in the gain region, and
the standing accretion shock instability) that increase the dwell time of
material in the heating region.
...
effectively a free parameter of most nucleosynthesis calculations, because it
emerges from a multi-dimensional simulation nobody runs inside a nucleosynthesis
study. Material just inside the cut is the most neutron-rich (deepest, most
processed), so the cut's location strongly affects the ejected iron-peak
composition and the ⁵⁶Ni mass.
...
Boltzmann problem (3 space + 2 angle + 1 energy) per flavour. Practical schemes
truncate it: two-moment (M1) closures, ray-by-ray approximations, IDSA, or full
Boltzmann in reduced dimensionality. A 3D simulation of one progenitor to ~1 s
post-bounce costs of order 10⁷ core-hours [typical]. **You cannot run a grid of
those.**
...
|---|---|---|
| **1D with parametrised heating** (PUSH, STIR, "neutrino light-bulb") | run 1D and tune a heating term so that the model explodes when a calibration set says it should | hours per progenitor — grids of hundreds are routine |
| **semi-analytic criteria** | compare a heating timescale with an advection timescale, or evaluate a critical-luminosity condition; the **compactness** ξ_2.5 (§0.4.3) is the crudest of these | seconds |
| **full multi-D** | the real calculation | 10⁶–10⁷ core-hours each |
...
whether a given star explodes depends on whether the accretion rate drops fast
enough at the right moment, which depends on the shell structure, which is
non-monotonic in progenitor mass. The outcome map "initial mass → explodes or
not" is famously **islands, not a boundary**.
...
used.** Those grids run stellar evolution to core collapse hundreds of times —
i.e. they pay the network cost hundreds of times, in the phase where it dominates
(§0.8.2). **That is the deployment story with the clearest economics**, and it is
...
$$
{}^{56}\mathrm{Ni} \xrightarrow{\ \tau_{1/2} = 6.08\ \mathrm{d}\ } {}^{56}\mathrm{Co}
\xrightarrow{\ \tau_{1/2} = 77.2\ \mathrm{d}\ } {}^{56}\mathrm{Fe} .
$$

Both steps are weak, and they are **not the same weak channel**: ⁵⁶Ni decays by
electron capture essentially 100% (it is not a β⁺ emitter), while ⁵⁶Co is ~81% EC
and ~19% β⁺ [sourced — verify]. Given how carefully Tier 2 §I.5 separates the
...
ε_Co ≈ 6.8 × 10⁹ erg g⁻¹ s⁻¹ at t = 0 [sourced — verify]. **Arnett's rule** — the
statement that at light-curve peak the luminosity approximately equals the
instantaneous radioactive energy deposition rate — then gives, for
M(⁵⁶Ni) = 0.07 M⊙ [typical] and a peak time ~18 days:
...
[derived here — right order of magnitude for a stripped-envelope event]
...
is powered by hydrogen recombination — there ⁵⁶Ni sets the post-plateau
radioactive tail and modulates the plateau *length*, not the peak luminosity.
...
reaches ⁵⁶Ni rather than ⁵⁴Fe or ⁵⁸Ni** (§0.3.3), so the ⁵⁶Ni mass — the most
robustly measured nucleosynthetic quantity in supernova astronomy — is a
readout of the neutron excess of the burnt material.
...
- **⁵⁷Ni/⁵⁶Ni** (observed as ⁵⁷Co/⁵⁶Co in the late light curve, since ⁵⁷Ni →
  ⁵⁷Co → ⁵⁷Fe). ⁵⁷Ni has one extra neutron, so its yield relative to ⁵⁶Ni is a
  nearly linear function of the neutron excess η = 1 − 2Yₑ. Measured in SN 1987A
  from the light-curve tail shape.
- **⁵⁸Ni/⁵⁶Fe** and **⁵⁴Fe/⁵⁶Fe** in the solar abundance pattern. The Sun's
  ...
  classic failure mode of supernova nucleosynthesis models, and it is a direct
  Yₑ diagnostic.
- **⁴⁴Ti**, produced in α-rich freeze-out (§0.3.5) and observed as γ-ray lines
  from Cas A and SN 1987A. Sensitive to the freeze-out conditions and hence to
  the shock energetics and mass cut as well as to Yₑ.
...
relative error in the observable. The observations are at percent precision.

[The η-linearity claim is an approximation valid for the mildly neutron-rich
isotopes; it is not a substitute for the missing derivative in §0.6.5. Tagged
**[derived here — approximate]**.]
...
dominate iron production but on a delayed timescale. The observed [α/Fe] versus
[Fe/H] trend in Galactic stars — a plateau at low metallicity followed by a
downturn — is the fingerprint of that timing difference.

The relevance here: **GCE models take supernova yields as inputs, integrated over
an initial mass function.** The yields are computed from progenitor models whose
...
events across Kamiokande II, IMB and Baksan), which confirmed the basic energetic
picture — total neutrino energy ~3 × 10⁵³ erg over ~10 s, matching §0.5.6's
neutron-star binding energy to within the uncertainties. Its light curve gave
M(⁵⁶Ni) ≈ 0.07 M⊙ [typical], and its late-time tail constrained ⁵⁷Ni/⁵⁶Ni.

**Cassiopeia A.** A ~350-year-old remnant, spatially resolved, with X-ray and
γ-ray maps of individual elements including ⁴⁴Ti. It shows that the ejecta are
strongly asymmetric and that the iron-peak material is not distributed as a
spherical model would predict — a reminder that the one-dimensional chain in §0.5
is a caricature of a three-dimensional process.
...
**FFN → LMP weak-rate-library shift** (5 × 10⁻³ to 1.5 × 10⁻² per trajectory) by
N ≈ 1.6 × 10³ steps under a systematic-accumulation assumption (Tier 2 §0.3.4).
...
|---|---|---|---|
| 1 | **3D convection vs MLT** — entrainment, shell mergers, the burning/mixing competition | qualitative changes to shell structure and yields; **not expressible as a δYₑ** | §0.2.5; unquantified anywhere |
...
| 4 | **weak-rate library choice** (FFN → LMP) | **5 × 10⁻³ – 1.5 × 10⁻² per trajectory** | measured, the project's own floor |
| 5 | **network size** (approx21 vs mesa_151) | the term this project removes: 377–651% Yₑ improvement, i.e. baseline error ≈ 2.4 × 10⁻³/step | **[RESULTS]** 2026-07-08, §VI.2 |
| 6 | **rate uncertainties within a library** (Hauser–Feshbach factor ~2 on unmeasured channels) | unpropagated | `tier1.md` §VIII.C.1 |
...
This is measured, not assumed: the left null space of ν restricted to strong/EM
columns is exactly 2-dimensional and equals span{A, Z} (Tier 2 §I.2.1). So
...
1. **Invariant #2 is structural, not stylistic.** Masking a weak column as
   ...
   balance in this regime — the inverse of an electron capture is a positron
   capture on a nucleus in a plasma with essentially no positrons at the relevant
   energies, plus a neutrino that has already left. There is nothing to balance
   against.
...
3. **The signal is small and rides on top of large strong-sector activity.** The
   ...
   exactly invariant; the weak sector moves Yₑ by ~10⁻³ over a trajectory. A
...
### 0.7.2 Electron capture in a degenerate plasma — why it is a one-way ratchet
...
Q_EC — and E_F rises with density as ρ^{1/3} (§0.1.4).
...
exceeds threshold [derived here — the μ_e⁵ scaling is the standard degenerate
limit; Tier 0 §0.1.5 does this properly]. The practical consequences:
...
- **Rising density switches EC on hard.** As the core contracts, E_F rises,
  ...
  of magnitude. Yₑ falls faster as the collapse proceeds — the runaway of §0.5.2.
- **β⁻ decay is the only restoring channel**, and it is **Pauli-blocked** in a
  degenerate plasma: the emitted electron must find an empty state above E_F, and
  there are few. So the restoring force is suppressed exactly where the driving
  force is strongest.
- **Therefore Yₑ ratchets downward.** It essentially never goes back up in this
  regime. Any error in the weak sector accumulates in a preferred direction —
  which is precisely the "systematic" row of the accumulation trichotomy
  (Tier 0 §IV.3) and the reason the per-step gate is set by N·δ rather than
  √N·δ (Tier 2 §0.3.4).
...
set **[RESULTS]** 2026-07-08. It has the ratchet and not the restoring channel,
in an even more extreme form than the physics itself imposes. Tier 0 §I.4 asks
...
A stellar-evolution code advances a 1D model of ~10³–10⁴ zones. Per timestep, per
zone, it must:
...
Step 4 is the one this project replaces. It is *operator-split* from the rest:
the network is integrated at fixed (T, ρ) over the timestep, and the structure
solve then uses the result (Tier 0 §IV.1). That split is what makes the network
call a well-posed map
...
— an O(Δt²) commutator term — whose size relative to the emulator's error nobody
has measured (Tier 2 **R-11**). If the split error dominates, the 3 × 10⁻⁶ gate
...
- **Stiffness forces implicit integration.** The stiffness ratio reaches
  10¹²–10¹⁵ (Tier 0 §V.3). Explicit methods would need ~10¹² steps per physical
  step. This is not an optimisation choice.
- **Each implicit step costs a Jacobian build and a dense-ish LU.** n³/3 FLOPs:
  1.7 × 10⁵ for n = 80, 1.1 × 10⁶ for n = 151 (6.5×), 2.8 × 10⁶ for n = 204
  (17×). Sparsity does not rescue it, because free nucleons couple to nearly
  everything and LU fill-in destroys the structure.
- **Every zone, every step**, 10³–10⁴ × 10⁶.
...
and structurally incapable of representing the neutron-rich β-decay chains.

The measured size of that compromise is the baseline this project's target is
defined against: the NNN paper reports (and this repo reproduced, **[RESULTS]**
2026-07-08) that a large-network emulator improves Yₑ accuracy over `approx21` by
**377–651%** on mesa_80 and **277–390%** on mesa_151. That is the error a real
...
| **QSE-reduced networks** (Hix & Thielemann) | solve equilibrated groups algebraically, integrate only bridges | correct in principle; requires the QSE assumption to hold — and this tier measures that it does **not** hold cleanly on the label manifold (§IV.5) |
| **adaptive networks** | activate species dynamically | reduces cost but not the stiff solve; bookkeeping-heavy |
| **NSE tabulation** | above some T, replace the network with a table X(T, ρ, Yₑ) | works only where NSE genuinely holds; the transition handling is exactly where this tier found the labels pathological (§III) |
| **direct ML emulation** (NNN, this project) | learn Φ_Δt | needs the accuracy and the invariants; no free lunch on rollout stability |

**What ML brings that the others do not** is the cost scaling. A GNN forward pass
costs O(K(n + m)d²) — linear in graph size and **independent of stiffness**. The
...
1. **Exact conservation by construction** rather than by penalty — dY = νΦ
   through a fixed integer stoichiometric matrix, so mass, charge-to-lepton
   closure and lepton number hold to float64 for *any* Φ, including an untrained
   network's. Measured: drift exactly 0.0 on column tests, ≤ max(10⁻¹²s,
...
2. **A flux-space target**, which makes errors attributable to named reactions
   ...
   which is §IV.
3. **A graph representation whose node features are (Z, N)**, making size
   transfer between networks a well-posed question rather than a retraining
   exercise.
...
measured what fraction of a real MESA run is spent inside `net`.**
...
and MESA reports per-module timing natively. Running a 20 M⊙ model to core
...
- the **scaling with network size**, which is the actual quantity the "accuracy
  ...
  6.5× network cost, and what does that do to *total* runtime?
...
at p = 0.9 it is 10×. [derived here]
...
**2. e_nuc *and its partial derivatives*.** This is the requirement that is
completely absent from every document in the project, and it is not optional.
MESA solves the structure equations implicitly, and the energy equation contains
e_nuc. The Newton solve therefore needs
...
which MESA's own network returns alongside the energy. **A black-box emulator
must supply them too.** The good news is that this is a natural strength of a
...
**6. Memory and model size.** The NNN ships **23.8 GB of checkpoints** for 18
models. A stellar code that loads a model per network per Δt has a startup and
memory profile that matters on a shared cluster. One conditioned model (S19) is
not just more elegant — it is ~9× smaller, which is a deployment argument the
...
| Yₑ physics floor | 5 × 10⁻³ – 1.5 × 10⁻² | FFN → LMP weak-library shift | **solid as a floor**, but it is an *input* uncertainty used as an *output* tolerance (§0.6.5, **T-01**) |
| regime box T₉ 1.6–7.9 | | §0.1.3 virial estimate lands at T₉ ≈ 5.5 for a 1.4 M⊙ core at 10⁸; §0.2.1 puts Si ignition at 3–4 GK; §0.5.2 puts Fe dissociation above the box | **solid** for the support; the *measure* on the box is not (**R-07**) |
...
| **∂e_nuc/∂lnT, ∂e_nuc/∂lnρ** | the host's implicit structure solve needs them (§0.8.6.2) | **none** — not even specified as outputs (**T-25**) |
...
9. Why is β⁻ decay unable to undo electron capture in a degenerate core, and what
   does that imply about how an emulator's Yₑ error accumulates?
...
13. Above ~5–7 GK a stellar code replaces the network with an NSE solve. Give
    three consequences for this project, one of which concerns where the label
    pathology lives.
...
MESA r23.05.1's `net` module does the following per call [sourced — verify
against the module source; the behaviour is confirmed by the Fortran probe of
Tier 1 §VI.8 and by the rate cross-check]:
...
2. **Evaluate rates** — REACLIB fits for strong/EM channels (Tier 1 §II),
   tabulated weak rates on a (T, ρYₑ) grid with the weaklib precedence
   LMP > Oda > FFN (Tier 1 §V), and reverse rates by detailed balance with
   partition functions (Tier 1 §III).
3. **Apply screening** — `chugunov` in this configuration, established by
   measurement to be `chugunov_2007` in pynucastro's naming: median ratio
   0.99999, max |log₁₀| = 0.0021 over all strong pairs × the cross-check grid
   **[RESULTS]** 2026-07-10.
4. **Integrate implicitly** with tolerances `eps` and `odescal`.
...
A MESA `net` call returns, per zone [sourced — verify against `net_lib.f90`; the
list is confirmed by what bbq writes and by what the structure solve consumes]:

| returned | consumed by | emulator status |
|---|---|---|
| the new composition | the EOS, opacities, mixing, the next call | **Target A's output** |
| `eps_nuc` | the energy equation | specified (invariant #5) |
| `d_eps_nuc_dT`, `d_eps_nuc_dRho` | **the structure solve's Jacobian** | **unspecified** (**T-25**) |
| `d_eps_nuc_dx(:)` | the same Jacobian, composition part | unspecified |
| neutrino losses | the energy equation, separately | specified, unsupervised (**T-17**) |
| a convergence / retry flag | the timestep controller | **has no analogue** (**T-26**) |

Three of the six rows have no counterpart in anything this project has specified,
and they are not exotic — they are what makes an implicit stellar-structure solve
converge. The composition and the energy are the *visible* outputs; the
derivatives and the failure signal are the ones that make the host work.
...
installed. A model that predicts composition to 10⁻⁶ and returns no
∂e_nuc/∂lnT cannot be dropped into MESA at all — the host would have to
finite-difference it, which costs two extra forward passes per zone per step and
gives back a noisy derivative, destroying both the speedup and the convergence.
...
## I.3 bbq's four modes, and why only one can make a trajectory

`bbq` (github.com/rjfarmer/bbq, pinned at commit 9783df31) is a thin driver
around MESA's `net` module. Its source has four `lib_*` files, and choosing among
them is the whole of ADR 0005's first decision.

| mode | file | behaviour | can it produce a trajectory? |
|---|---|---|---|
| `use_input_file` | `lib_bbq.f90` | burn each input row **independently** for its own duration | **no** — no state carry-over |
| `use_sampling` | `lib_sampler.f90` | sample states from a distribution and burn each | no |
| `use_random` | `lib_random.f90` | random states | no |
| `use_profile` | `lib_profile.f90` | burn a MESA profile's zones | no — and **writes no eps_nuc** |
| `use_hydrostatic` | `lib_hydrostatic.f90` | burn a sequence of durations **carrying composition forward**, one output row each | **yes** |

Only `use_hydrostatic` gives (i) composition carry-over between rows, (ii)
per-row eps_nuc, and (iii) exact cadence control, because each line of
`times.txt` is a per-step burn *duration*. The shipped test trajectories have
`lib_hydrostatic.f90`'s exact header — `age dt eps_nuc eps_neu <isos>` — which is
how the project knows which mode generated them [sourced — the source read is
...
&bbq
   ...
   stptry = 0            ! let the solver pick the first step
...
composition. Everything this project calls a trajectory is a **constant-(T, ρ)
relaxation**, which is a specific and limited object; §IV.3 is where that limit
...
1. **A Sobol grid over the regime box.** 2²⁰ = 1,048,576 points in
   ...
   Sobol sampler. This is the single most consequential engineering fact in the
...
4. **Test trajectories** — 1508 constant-(T, ρ) hydrostatic runs per network,
   shipped as `output_T_<logT>_rho_<logRho>.txt`.
...
analysis: the Yₑ = 0.45 target is **unreachable** by any two-isotope mix of
abundant A ≥ 12 species, because the network's Z/A floor there is ²²Ne at
0.45455. The campaign substitutes 0.455 and **records the substitution in the
...
Two engineering points from `run_campaign.py` worth carrying:
...
clean data**. The single-Si-cluster QSE reference of Hix & Thielemann does not
describe this manifold. Group-*organised* structure exists — non-group spread is
...
in erg per gram. [derived here] This is **exact and quadrature-free**: it uses
only the endpoint compositions and a table of nuclear masses. It does not care
about rates, screening, the solver, or the timestep. That independence is what
...
reactions are slow, one-directional, and not in balance (§0.7.1) — so once the
strong pairs equilibrate, composition changes at the weak timescale, which is
long. Those tail rows are legitimate relaxed states of the label dynamics
...
pynucastro nuclear inputs (Rauscher partition functions, binding energies, spins)
...
construction — MESAHub/mesa issue **gh-575**, fixed in release 24.08.1
[sourced — verify: the changelog text quoted below is recorded in **[RESULTS]**
2026-07-09].
...
Tier 1 §III derives why that factor is there: the reverse of an
...
**The chapter-8 row is the one that matters here, and it is beyond what the NNN
paper's own Appendix B describes.** The paper lists light-sector channels; this
project found that the 1 → 3 photodisintegration reverses — including **c12 → 3α,
...
**This also explains the κ floor.** Tier 2 §IV.8b's Wegscheider measurement found
Q ∉ rowspace(ν) for the strong sector and predicted a κ data floor of
...
one variable changed, and the outcome switches from "reproduces the shipped
...
audit of a labelled dataset, and it is not standard practice anywhere in
scientific ML.
...
**A factor of 25 in the Yₑ change.** The reason is direct: electron-capture rates
are per-nucleus and species-specific (§0.7.3), so *which species host the
...
labels were generated with the authors' *locally fixed* MESA (the paper's
Appendix B says so), so labels are clean and *our* stock MESA is the outlier.
That premise was reasonable, sourced, and recorded. It is now known to be false
...
docs/phase0-checklist.md and the verdict document, never here."* Five functions:
...
| `cond_s_active` | cond of ν restricted to active columns, **rank-revealing** (ratio of extreme *nonzero* singular values) |
...
The Guidry departure criterion (Tier 2 §III.10) declares a species equilibrated
...
criterion tests against. The two are 6.7–13.0 dex apart at T₉ ≳ 5 and displaced
...
unsourced. Measured as log₁₀[(fastest κ-balanced gross rate)/(slowest
...
(the Ono & Sugimura timescale-rescaled update) has "a separated fast sector" as
its stated premise. That premise is now measured false. `tier1.md` §VIII.F ranks
...
- *The independent-leg check* (engine-rate energy vs label-ΔX energy, §II.3):
  median ratio **0.979 / 0.996**. mesa_80 at a 2.1% median is **outside** a 1%
...
6.5–18.6.** So `RESULTS.md`'s "657k carrying-pair samples" carry roughly **300
independent systems**.
...
**1. Cluster bootstrap over trajectories.** Resample *trajectories* with
...
Expected outcome, predicted here so it can be checked: with n_eff ≈ 300 the
95% interval on a fraction near 0.8 is roughly ±0.045 under independence and
**±0.12 – ±0.19** at design effects 6.5–18.6 [derived here, from
√(p(1−p)·deff/n)]. So the §IV.6 coverage numbers — 0.62, 0.80, 0.95 — are
...
- coverage rises steeply as θ falls → the failing species' carriers sit just
  ...
  entropy-weighted active set (σ_j = 2k_B s_j κ_j artanh κ_j, parameter-free)
...
top-20 weak channels carry 1.00 of the Yₑ budget — and that concentration is
...
on a transformed quantity that has no physical meaning.

NuGNN (Kim et al. 2026, arXiv:2606.04491) [sourced — verify] documents exactly
this instability: log-space conservation handling that does not conserve. The
repo encodes the lesson as a refusal in `graph/projector.py`'s docstring and as
invariant #4.
...
augmented state u = [Y, Φ] with dΦⱼ/dt = Rⱼ, integrated by BDF with a sparse
...
Compositional data analysis (Aitchison) treats data on a simplex through log-ratio
transforms — additive (alr), centred (clr) and isometric (ilr) — under which the
...
- The NNN's log-softmax output (§VI.1) is precisely the **inverse clr/alr map**
...
- The Aitchison geometry's pathology is also known: **zeros are not
  ...
  sub-literature on zero replacement. This project's data has a hard floor at
...
where it reaches −19.2% on ⁴⁰Ca(α,γ)⁴⁴Ti at T₉ = 7.9. So an open Tier-1 item and
...
Grichener, Farmer, Bauer et al. 2025, *Nuclear Neural Networks* (ApJS 279, 49;
arXiv:2503.00115; Zenodo 14873443) [sourced — verify]. Read from the shipped
code (`repro/nnn/patched/NNNfunctionsCompPlusEps.py`) rather than from the paper:

**Architecture.** A plain feed-forward MLP:

```
  input  (n_iso + 2)  ──▶ Linear 128 ──ReLU──▶ Linear 256 ──ReLU──▶
                          Linear 256 ──ReLU──▶ Linear (n_iso + 2)
```

Four layers, widths 128 / 256 / 256 / out. Input = the initial mass fractions
plus log T and log ρ (dims 82 / 153, confirmed against the checkpoints
**[RESULTS]** 2026-07-08). Output = the final mass fractions plus e_nuc and ε_ν.
...
construction**. The NNN *does* conserve mass — through a normalisation, not
...
input, it is a model *index*. Eighteen trained models ship (23.8 GB of
checkpoints).
...
paper reports a Yₑ *loss* as a separate metric rather than an invariant, and why
the reproduced relative Yₑ error is 0.33–0.54% (mesa_80) / 0.55–0.73% (mesa_151)
**[RESULTS]** 2026-07-08.

Put that number in the project's own units. Yₑ ≈ 0.47, so 0.5% relative is
**|ΔYₑ| ≈ 2.4 × 10⁻³ per step**. The project's per-step gate is 3 × 10⁻⁶.
**The baseline is ~800× outside the gate this project sets itself.** That is a
...
error on a 10⁻¹⁴ species the same as on a 0.5 species — which is why the paper's
per-isotope error band is 10⁻⁴ … 10⁻¹ with light isotopes *best* — and it means
...
| per-isotope mean ΔX band | 90% of isotopes in [10⁻⁴, 10⁻¹], median 3.2 × 10⁻³ | most in that band |
...
Kim et al. 2026 (arXiv:2606.04491) [sourced — verify] is the nearer prior art: a
graph neural network for nuclear networks, hence the same architectural family as
this project.

Its documented failure mode is the one that shaped this project's invariant #4:
**log-space conservation handling that does not conserve** (§V.3). That is a
useful competitor to have — it establishes that the GNN framing is live and
...
|---|---|---|---|
| architecture | MLP | GNN | GNN, heterogeneous, (Z,N) node features |
| conservation | ΣX = 1 by softmax | attempted in log space, documented unstable | **exact by construction, all three laws** |
| target | final X | ΔX | **per-reaction flux Φ** |
| Δt handling | 9 models | — | one model, conditioned |
| size transfer | retrain | — | (Z,N) embedding — a stated falsifier |
...
**4. Rollout, not just one-step.** The NNN is evaluated one step at a time. The
...
**Baselines**, all of them (**T-08**): the NNN, `approx21`, persistence,
nearest-neighbour, and **NSE** — the last on the hot strata where §0.3.6 says it
is the real incumbent.
...
1. **Conservation by construction works and costs nothing.** dY = νΦ through a
   ...
   within max(10⁻¹²s, 10⁻¹³G) over twelve decades of flux magnitude, with dYₑ
...
4. **The bridge structure is real and concentrated.** Top-20 inter-group columns
   carry 0.88 of the flux; the literature bottleneck ⁴⁵Sc(p,γ)⁴⁶Ti is recovered
   independently at rank 2/251. BCF68's picture survives on relaxed states.
   **[RESULTS]** 2026-07-12.
...
| ∂e_nuc/∂lnT, ∂e_nuc/∂lnρ | an output nobody specified; **MESA cannot run without it** (**T-25**) |
...
project is a consequence.* ²⁸Si cannot fuse with ²⁸Si at any temperature the star
survives, so the flow proceeds by disassembly and reassembly through a
...
| **T-02** | mesa_80 has the EC ratchet and **0/8** β-decay partners. The predicted Yₑ bias direction vs mesa_151 has never been tested on the shipped labels | [§0.7.2](#072-electron-capture-in-a-degenerate-plasma--why-it-is-a-one-way-ratchet) | ~2 h | a physics-level validation (or refutation) of the size-transfer premise, on data already on disk |
...
| **T-05** | No kill-test statistic carries a cluster-robust interval; ICC 0.22–0.70, n_eff ≈ 300 not 657k | [§IV.8](#iv8-the-verdict-and-the-statistics-it-does-not-carry) | ~1 h | supplies the missing uncertainty on every headline number. Extends **R-08** |
...
| **T-15** | **No solver-independent ground truth.** Every check compares MESA against pynucastro-derived rates — now known insufficient, since MESA was *wrong* | [§III](#part-iii-s13--silicon-burning-on-real-histories-and-the-label-pathology) | ~1 wk | SkyNet/WinNet would have caught the pathology directly. Elevates `tier1.md` §VIII.E.3 from debt to priority |
...
| **T-21** | **The NSE transition is unexamined.** Host codes replace the network with a Saha solve above ~5–7 GK — i.e. exactly where the labels are pathological and where the emulator's real competitor is already fast and exact | [§0.3.6](#036-the-nse-transition--where-a-network-is-replaced-by-a-table) | ~2 h | tips the benchmark-vs-physics fork decisively toward **T-03**; identifies a much smaller (T,ρ,Yₑ)→dYₑ/dt emulator as possibly the valuable object |
...
| **T-37** | **"Calibrated" has a literature the project has not named.** Deep ensembles are both the most expensive UQ (**R-12**'s critical path) and the worst-calibrated under the distribution shift §IV.3 *measured*. Conformal prediction is the tool; its exchangeability assumption fails in two already-quantified ways | [§X.5](#x5-what-calibrated-means-for-a-deterministic-map) | ~½ d | **weighted conformal needs exactly R-07/T-13's density ratio** — two unrelated-looking items are one measurement. And the project already computes four free physics-residual OOD scores (entropy sign **R-05**, energy consistency, positivity **R-04**, fixed-point distance **T-33**) without labelling them as such |
...
| **T-41** | ⚠ **An unretired design premise, found while sweeping**: the architecture report's "⁴⁵Sc(p,γ)⁴⁶Ti carries **≈75%** of the inter-group flow" versus the measured **share 0.10–0.12, rank 2/251, top-20 carry 0.88**. Both documents report this as CONFIRMED — which it is *qualitatively* and is not *quantitatively* | [§X.10](#x10-one-unretired-premise-found-while-sweeping) | ~1 h | second instance of a named pattern — **an unsourced magnitude attached to a correct qualitative claim** (the first was "6–8 orders", retired §IV.7). Check provenance, retire with the measured replacement, then audit the remaining inherited premises for others. A `referee` task |
...
| 4 | **T-08 + T-21** — the real incumbent | ~1 h | Run NSE, persistence and nearest-neighbour as baselines; and notice that above 5 GK a host code would use a Saha solve, not a network. Together these may redefine the emulator's domain *and* its competitor, which is a scoping result, not a measurement. |
...
| 6 | ⚠ **T-25 + T-26** — the interface requirements that are not optional | design + ~1 d | An emulator with no ∂e_nuc/∂lnT cannot be installed in MESA at all, and removing the network's convergence failure silently breaks the host's timestep controller. Both respecify S22 *before* it is built. **T-25** also promotes `tier1.md` §VIII.E.2 from an accuracy item to a prerequisite. |
...
flip, the probability that one wins **at least 60 of 108** cells is about 5%, and
the probability of winning at least one cell "by a lot" is essentially 1. With
...
- **Batch-size-dependent output.** Many frameworks produce bitwise-different
  ...
  orders). A stellar code that batches a variable number of zones per timestep
...
2. The *unknown* systematics are the Hauser–Feshbach rate uncertainties on
   ...
   experimentally inaccessible ones, and the mass/Q differences, which Tier 1
   §VIII.C.5 notes enter as e^{−Q/kT} with Q/kT up to several hundred — a 100 keV
   Q difference is a **47% rate difference** at kT = 0.26 MeV.
...
one-parameter family, since the strong sector cannot move Yₑ (§0.7.1). Include
...
Jacobian has eigenvalues ≈ e^{λ_i Δt}. With most λ_i strongly negative and
Δt large:
...
|---|---|---|
| **under-contraction** — learned singular values > 1 where the true map has ≈ 0 | the model preserves directions physics destroys | rollout amplifies its own error along stiff directions: the classic autoregressive blow-up, and precisely what S20's pushforward loss and noise injection are *heuristics* for |
...
and the network's output enters the residual (through e_nuc, the composition, and
the EOS's dependence on Ā and Z̄). Newton needs a residual function that is
...
well-known pathology with discontinuous EOS tables and phase transitions, and it
is exactly reproduced here.
...
> all — only the product ρΔt would matter.
...
1. **A better conditioning variable.** The model is currently conditioned on
   ...
   sector — the numerical majority of columns (ch. 4–5 are 397/979 of the
   reactions **[RESULTS]** 2026-07-08) — in one variable, leaving the network to
...
- **Statistics.** Deep ensembles are known to be **poorly calibrated precisely
  under distribution shift**, and distribution shift is not a risk here — it is
  a *measured fact* (§IV.3). The consolidated architecture report already names
...
**The tool the project has not named is conformal prediction**, which gives
...
|---|---|---|
| rows within a trajectory are correlated | ICC 0.22–0.70 (**T-05**) | block / cluster conformal — calibrate over **trajectories** |
| the deployment measure ≠ the training measure | §IV.3, **R-07**, **T-13** | **weighted** conformal, which needs the likelihood ratio between the two measures |
...
NNN's grid, which was inherited from a design choice nobody in this project made.
**Nobody has measured the distribution of Δt that MESA actually takes during
silicon burning.**
...
*shorter* than 10⁻⁶ s — plausible near core collapse — the emulator is being
extrapolated from its first step, and §III.1's finding that even the shortest
...
**Network switching mid-run.** MESA can change nets during an evolution (a star
does not need an iron-peak network during hydrogen burning). An emulator trained
...
inter-group carriers with **share 0.10–0.12** under the `a24_46` boundary, and
the **top-20 columns carry 0.88**.
...
Three things should happen: check the 75% figure's provenance (it may be a
different quantity — a specific state, a different group boundary, or the flow at
one edge rather than the total); if it does not reproduce, retire it in
`RESULTS.md` with the measured replacement, exactly as the timescale figure was
retired; and audit the remaining inherited design premises for others of the same
kind. The two found so far were both **unsourced magnitudes attached to correct
qualitative claims** — a specific and recurring failure pattern worth naming.
