...
| date | quantity | value | tag | script | code version | data version |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-12 | κ↔δ on the relaxed manifold | Spearman(log κ unscr, log δ_r) = **+0.491 / +0.498** over 657k/1.5M carrying-pair samples (mesa_80/151; Step-5 shipped-only pre-arrival subset sharpens to +0.650/+0.666) — κ and Guidry-δ agree on ORDERING but δ (vs true NSE) never crosses the mask thresholds on this manifold | measured | scripts/run_killtest.py --relaxed --include-reruns | c7a3b2e | flux stores 9894a162/1043ba82/956f424a (+151 twins) |
| 2026-07-12 | Guidry ε sweep {3e-3, 1e-2, 3e-2} | maskable columns per row: median **0 at every ε, both nets** (p90 = 0) — the label manifold NEVER reaches Guidry equilibrium vs true NSE (its own equilibria are Appendix-B-displaced; RESULTS 2026-07-11). Consequently cond(S_active) = cond(full ν) = **41.8 / 57.4** at every ε (≪ 1e6 gate, PASS) and the hybrid design's maskable set is EMPTY: Target A operates full-width on this data | measured | ditto | c7a3b2e | ditto |
