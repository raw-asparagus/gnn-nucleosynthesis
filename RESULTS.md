...
## Kill-test Task 2B: the relaxed manifold (2026-07-12, Step 6)

Verdict distribution: pre-terminal rows of 20 shipped + 209 rerun
trajectories per net (4,693 rows/net sampled ≤24/trajectory), UNSCREENED
flux twins, δ_r vs the independent Saha NSE per row, phase split at the
first-quiet arrival row (relaxing vs attractor). Script:
scripts/run_killtest.py --relaxed --include-reruns (commit c7a3b2e);
outputs archived in the step-6 scratch logs, regenerable from the hashed
flux stores.

| date | quantity | value | tag | script | code version | data version |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-12 | κ↔δ on the relaxed manifold | Spearman(log κ unscr, log δ_r) = **+0.491 / +0.498** over 657k/1.59M carrying-pair samples (mesa_80/151; Step-5 shipped-only pre-arrival subset sharpens to +0.650/+0.666) — κ and Guidry-δ agree on ORDERING but δ (vs true NSE) never crosses the mask thresholds on this manifold | measured | scripts/run_killtest.py --relaxed --include-reruns | c7a3b2e | flux stores 9894a162/1043ba82/956f424a (+151 twins) |
| 2026-07-12 | Guidry ε sweep {3e-3, 1e-2, 3e-2} | maskable columns per row: median **0 at every ε, both nets** (p90 = 0) — the label manifold NEVER reaches Guidry equilibrium vs true NSE (its own equilibria are Appendix-B-displaced; RESULTS 2026-07-11). Consequently cond(S_active) = cond(ν restricted to net columns) = **41.8 / 57.4** (full ν: 41.6 / 57.9) at every ε (≪ 1e6 gate, PASS) and the hybrid design's maskable set is EMPTY: Target A operates full-width on this data | measured | ditto | c7a3b2e | ditto |
| 2026-07-12 | κ-balance census | strong pairs with κ < 1e-3: ≤ 0.4% of carrying pairs in every stratum/phase — clean detailed balance essentially never occurs; the cancellation structure is a CONTINUUM of κ ∈ 1e-3…1e-1 pairs (frac κ > 0.1 drops to 0.70/0.64 at T9 [5.0,6.3)) | measured | ditto | c7a3b2e | ditto |
| 2026-07-12 | THE 95% GATE (κ-active {κ>0.1} coverage of dominant-isotope \|ΔX\| turnover, X > 1e-3) | SPLIT VERDICT: median-dominant-isotope coverage ≥ 0.95 in most strata/phases (1.00 typical) EXCEPT T9 [5.0,6.3): med-cov median 0.80/0.62 (relaxing) and 0.62/0.73 (attractor); WORST dominant isotope per row (min-cov median): 0.90/0.75 at [1.6,2.5) (relaxing phase) falling to **0.0022–0.1658 (mesa_80) / 0.0084–0.0700 (mesa_151) across [4.0,6.3) strata × phases** ([4.0,5.0) relaxing: 0.1658/0.0700; [5.0,6.3): 0.0022–0.0290/0.0084–0.0475) — some dominant species' net evolution rides on near-cancelled columns (1/κ error amplification 10–10³ on gross-scale errors); these carriers are FEW and identifiable (see spread row). [5.0,6.3) is also where label physics is bug-displaced | measured | ditto | c7a3b2e | ditto |
| 2026-07-12 | spread check (FAIL if > ~30% of net columns near the κ floor) | low-κ (κ ≤ 0.1) column share of active net columns: median 0.9%–24% across all strata/phases (worst: attractor [5.0,6.3) 0.24/0.21) — **below the ~30% FAIL line everywhere**; no diffuse-spread failure | measured | ditto | c7a3b2e | ditto |
| 2026-07-12 | \|dẎₑ\| structure | coverage by any active set = 1.0000 STRUCTURALLY (weak columns never maskable, invariant #2); top-k concentration on high-Yₑ QSE-window rows: top-5 = 0.96 / 0.75, top-20 = 1.00 / 1.00 (mesa_80/151; #1 Ni56 EC 0.62 / 0.28, then S31 EC, Fe52 EC, p EC) — loss-weight targets confirmed | measured | ditto | c7a3b2e | ditto |
| 2026-07-12 | bridge sets FINALIZED (full relaxed manifold, both boundary variants) | mesa_80: Ne22(α,n)Mg25 (0.20), Al27(p,α)Mg24 (0.13), P31(p,α)Si28 (0.11), Na23(α,p)Mg26, Mg24(n,γ)Mg25 — top-20 carry 0.88; mesa_151: Mg24(p,α)Na21 (0.14), Mg24(p,γ)Al25, Cl35(p,α)S32, P31(p,α)Si28, Ca44(p,γ)Sc45; **⁴⁵Sc(p,γ)⁴⁶Ti rank 2/251 (share 0.10–0.12) under a24_46** — Step-5 preliminary sets confirmed; boundary variant changes top-k by ≤ 0.02 (no verdict sensitivity) | measured | ditto | c7a3b2e | ditto |
| 2026-07-12 | timescale separation (fastest κ-balanced gross / slowest 95%-bottleneck net) | phase-mixed distribution is BROAD and mostly ≤ 0: per-stratum medians −12.2…+1.4 dex (p10 −25, p90 +3.5) — the unsourced "6–8 orders" working figure is NOT SUPPORTED on the label manifold: there is no cleanly-separated fast equilibrated sector (κ-balanced pairs ≤ 0.4% and mostly low-flux). RETIRES the assumed figure (docs Rule 1 standing list) with a measured replacement: separation is weak-to-absent | measured | ditto | c7a3b2e | ditto |
| 2026-07-12 | Guidry-mask churn (QSE-window trajectories, per output step) | flips/step median 0.00–0.03 (churn fraction ≤ 0.002%/step) at every ε, both nets, n=35 trajectories — trivially below the 5%/step freeze trigger; but the mask is empty anyway (see ε-sweep row): the Component-B hybrid-vs-frozen decision is settled by EMPTINESS, not churn | measured | ditto | c7a3b2e | ditto |

## Integrated-flux label prototype: correctness, agreement, feasibility (2026-07-12, Step 6 Task 3)

Instrument: src/gnn_nucleo/fluxes/integrate.py (ADR 0006) +
scripts/step6_integrate_check.py (36/24 stratified Sobol states, ONE
integration to 1e2 s serving all nine measured dts; 11 workers; 4800-s
deadline, censored states are lower bounds).

| date | quantity | value | tag | script | code version | data version |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-12 | internal identities | ΔY = νΦ EXACT by construction (returned Y := Y0 + νΔΦ); solver-vs-identity residual 2.1e-16 molar (machine precision); baryon/charge closure by Cν = 0; RHS ≡ engine ≤ 1e-12 rel; BDF ≡ Radau ≡ BDF(rtol 1e-10) to 5e-8 rel on the stiffest test state | measured | tests/test_integrate.py | e2bb89c | pyna 2.12.0 |
| 2026-07-12 | ENERGY IDENTITY (invariant #5, checklist row 11) | flux route ΣQⱼΦⱼ vs mass-excess bookkeeping: median 6e-6…2.5e-5 rel, max 8.7e-4, **frac ≤ 1% = 1.000 in every measured stratum, both nets** (incl. the whole 3–4 GK band) — gate passed with ≥3 orders of margin | measured | scripts/step6_integrate_check.py | c7a3b2e | Zenodo 14873443 |
| 2026-07-12 | integrator ΔX vs shipped labels (handshake band, ε_lab = 1e-7, per T9 stratum × dt) | QSE window [3.3,5.0): median species-fraction in band 0.74–0.99 (best 0.99 at [4.0,5.0) dt 1e-1); cold strata 0.62–0.90 (rate-class differences through stiff free-nucleon relaxations; solver-independent); T9 ≥ 5: unmeasurable within deadline AND label-pathological (RESULTS 2026-07-11) — local Φ-label generation is PROVEN for T9 < 5 | measured | scripts/step6_integrate_check.py | c7a3b2e | ditto |
| 2026-07-12 | throughput / corpus feasibility | median wall ≥ 383 / 1519 s per state (mesa_80/151; 17/36 and 10/24 states CENSORED at 4800 s — every censored state is T9 ≥ 5); ≤ 9.4 / 2.4 states/h/core ⇒ corpus ≥ 110,647 / 439,302 core-h — **full-corpus local Φ supervision INFEASIBLE; stratified subsets (10³–10⁴ states, T9 < 5, QSE window first) feasible** (≈ 4–40 core-days). Hot-state cost cause diagnosed: the deliberately neglected tabular-EC ρYₑ Jacobian chain dominates the weak-drift phase at T9 ≥ 5 — the ADR 0006 contingency (add the Yₑ-chain term) is the Step-7 unlock for hot-strata Φ labels | measured | scripts/step6_integrate_check.py | c7a3b2e | ditto |
| 2026-07-12 | weak-column κ verification (referee-requested) | κ = 1.0 EXACTLY on all 8,825,170 / 33,189,419 (weak column × relaxed row) samples with f⁺ > 0 (both nets, both unscreened relaxed runs) — every contributing weak column is in {κ > 0.1}, so \|dẎₑ\| coverage by the κ-active set = 1.0 measured (fluxless weak columns have κ = 0 by the engine's zero-denominator convention and contribute nothing) | measured | inline verification over hashed flux stores (commit dff6bb3 session) | dff6bb3 | flux stores 9894a162/1043ba82/956f424a + 151 twins |
